...
(time falco -t 25 -o "${out}/${out}_raw_qa/1" $i1) 2>&1 | tee -a "${out}/${name}_long_log.log" 
echo "For read ${i2}" 2>&1 | tee -a "${out}/${name}_long_log.log"
(time falco -t 25 -o "${out}/${out}_raw_qa/2" $i2) 2>&1 | tee -a "${out}/${name}_long_log.log"
mkdir "${out}/${out}_trim"
...
echo "" 2>&1 | tee -a "${out}/${name}_long_log.log"
(time trimmomatic PE -phred33 -threads 25 $i1 $i2 "${out}/${out}_trim/r1.paired.fq.gz" "${out}/${out}_trim/r1_unpaired.fq.gz" "${out}/${out}_trim/r2.paired.fq.gz" "${out}/${out}_trim/r2_unpaired.fq.gz" LEADING:4 TRAILING:4 MAXINFO:97:0.7 MINLEN:100 AVGQUAL:28) 2>&1 | tee "${out}/${out}_trim/trim_log.txt"
...
(time falco -t 25 -o "${out}/${out}_raw_qa_trimmed/1" "${out}/${out}_trim/r1.paired.fq.gz") 2>&1 | tee -a "${out}/${name}_long_log.log"
echo "For 2nd trimmed paired read" 2>&1 | tee -a "${out}/${name}_long_log.log"
(time falco -t 25 -o "${out}/${out}_raw_qa_trimmed/2" "${out}/${out}_trim/r2.paired.fq.gz") 2>&1 | tee -a "${out}/${name}_long_log.log"
echo "______________________________________" 2>&1 | tee -a "${out}/${name}_long_log.log"
echo "" 2>&1 | tee -a "${out}/${name}_long_log.log"
...
(time megahit -t 25 -1 "${out}/${out}_trim/r1.paired.fq" -2 "${out}/${out}_trim/r2.paired.fq" -o "${out}/${out}_asm" --min-count 4) 2>&1 | tee -a "${out}/${name}_long_log.log"
echo "______________________________________" 2>&1 | tee -a "${out}/${name}_long_log.log"
echo "" 2>&1 | tee -a "${out}/${name}_long_log.log"
...
(time quast.py "${out}/${out}_asm/final.contigs.fa" -o "${out}/${out}_quast_out" -t 25) 2>&1 | tee "${out}/${out}_quast_out/asm_log.txt"
